...
from ringmomultimodal.utils import get_root_logger
from ..utils import PatchEmbed, PatchMerging, StageWrapper, ModelFlow, FlowBackBoneBase
...
class RingMoGiant(FlowBackBoneBase):
    ...
    def __init__(self,
        ...
        if stage_names is not None:
            assert len(stage_names) == num_layers, "The stage_names' length should be equal to depths'"
            self.stage_names = stage_names
        else:
            self.stage_names = ['extract_feat'] + [f"swin_stage_{i}" for i in range(num_layers)]

        super(RingMoGiant, self).__init__(init_cfg=init_cfg)
    ...
    def generate_stages(self):
        ...
        def pos_embed(x, **kwargs):
            x, hw_shape = self.patch_embed(x)
            if self.use_abs_pos_embed:
                x = x + self.absolute_pos_embed
            x = self.drop_after_pos(x)
            return x, {'hw_shape': hw_shape}

        pos_embed_stage = StageWrapper(pos_embed, self.stage_names[0])
        model_flow.append(pos_embed_stage)

        for stage_idx, stage in enumerate(self.stages):
            stage_name = self.stage_names[stage_idx + 1]

            def swin_block(x, hw_shape, i=stage_idx, **kwargs):
                stage_module = self.stages[i]
                x, hw_shape, out, out_hw_shape = stage_module(x, hw_shape)
                if i in self.out_indices:
                    ...
                    out = out.view(-1, *out_hw_shape,
                                   self.num_features[i]).permute(0, 3, 1,
                                                                 2).contiguous()
                return x, {'hw_shape': hw_shape, 'out_hw_shape': out_hw_shape,
                           "output": out} if i in self.out_indices \
                    else {'hw_shape': hw_shape}

            swin_block_stage = StageWrapper(swin_block, f'{stage_name}')
            model_flow.append(swin_block_stage)
        return model_flow
    ...
    def train(self, mode=True):
        ...
        super(RingMoGiant, self).train(mode)
        self._freeze_stages()
    ...
    def test_model_flow(self, x):
        model_flow = ModelFlow(self)
        return model_flow(x)
...
if __name__ == '__main__':
    model = RingMoGiant()
    print(model)
    x = torch.randn(10, 3, 800, 800)
    with torch.no_grad():
        model_flow_x, kwargs = model.test_model_flow(x)
        x, wh_shape = model(x)
        print(x == model_flow_x)
        print(kwargs['hw_shape'], wh_shape)
