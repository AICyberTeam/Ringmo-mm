...
class ShiftWindowMSA(BaseModule):
    ...
    def forward(self, query, hw_shape):
        ...
        # print('x.shape before:', query.shape)
        # 新增高低频部分
        # if self.high_dim != 19:
        #     x_tmp = query  # 原始的特征图，准备给高低频提取
        #     # print('x.shape:', x.shape)
        #     # x_tmp = x_tmp.transpose([0, 3, 1, 2])
        #     # HighMixer
        #     hx = x_tmp[:, :self.high_dim, :, :]
        #     # print('hx.shape:', hx.shape)
        #     # exit()
        #     hx = self.high_mixer(hx)
        #     # B_tmp, H_tmp, W_tmp, C_tmp = hx.shape
        #     # hx = hx.view(B_tmp, H_tmp * W_tmp, C_tmp)
        #     # hx = hx.expand(B_tmp, H_tmp * W_tmp, C_tmp)
...
class SwinBlock(BaseModule):
    ...
    def forward(self, x, hw_shape):
        ...
        def _inner_forward(x):
            ...
            # 新增高低频部分
            # if self.high_dim != 19:
            x_tmp = x  # 原始的特征图，准备给高低频提取
            B, HW, C = x.shape
            # print('x/hx.shape before:', x.shape)
            x_tmp = x_tmp.view(B, self.high_dim, HW // self.high_dim, C)
...
class SwinTransformerMix(BaseModule):
